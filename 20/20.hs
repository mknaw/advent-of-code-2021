...
readBinary = foldl' (\acc elem -> 2 * acc + intify elem) 0
  where intify b = if b then 1 else 0
...
calculateRow' acc row
  ...
  where
      ...
      nextNumber | null acc  = readBinary . take 3 $ row
                 | otherwise = ((head acc .&. 3) `shift` 1) + nextBit
      nextRow    | null acc = drop 3 row
                 | otherwise = tail row
...
calculateCol acc col
  ...
  where
      nextNumber | null acc  = sum $ zipWith shift (take 3 col) [6, 3, 0]
                 | otherwise = ((head acc .&. 63) `shift` 3) + head col
      nextCol    | null acc = drop 3 col
                 | otherwise = tail col
...
enhance algorithm (image, borderValue) = (image', borderValue')
  where
    ...
    borderValue' = if borderValue
                      then algorithm ! 511
                      else algorithm ! 0

countLitPoints :: Image -> Int
countLitPoints image = sum . map sum $ intified
  where intified = (map . map) (\b -> if b then 1 else 0) image
...
litPointsAfter steps algorithm image =
  ...
  where enhance' = enhance algorithm
