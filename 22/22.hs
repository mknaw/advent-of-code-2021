...
readStep s = Step { _state=state' == "on"
                  , _xs=xs
                  , _ys=ys
                  , _zs=zs
                  }
...
boundStepsToArea (rX, rY, rZ) = map clamp . filter filterF
  where
    -- TODO this is UGLY
    filterF step =  inRange rX (fst $ _xs step) || inRange rX (snd $ _xs step)
                 && inRange rY (fst $ _ys step) || inRange rY (snd $ _ys step)
                 && inRange rZ (fst $ _zs step) || inRange rZ (snd $ _zs step)
    clamp step = step { _xs=clamp' (_xs step) rX
                      , _ys=clamp' (_ys step) rY
                      , _zs=clamp' (_zs step) rZ }
      where clamp' (a1, b1) (a2, b2) = (max a1 a2, min b1 b2)
