...
initGame players =
  ...
  where players' = M.fromList $ zip [1..] players
...
numberToDiceCombos = M.fromList [ (3, 1)
                                , (4, 3)
                                , (5, 6)
                                , (6, 7)
                                , (7, 6)
...
getNexts k = zip [3..9] nextPositions
  where nextPositions = take 7 . drop (k + 2) . cycle $ [1..10]
