  ...
  where
    ...
    validAmphipods =
      filter (\a -> not $ isDone a && roomIsReady as a)
        . concatMap (take 1 . reverse . mapMaybe (`atPosition` as))
        $ rooms
...
getDestination as a
  ...
  | otherwise = head t
  where
    t = filter (isUnoccupied as) (getTargetPositions a)
...
validMoves as a
  ...
  | _hasMoved a =
    if isStuckInHallway as a
      then []
      else map wrapInMove $ take 1 validTargets
  | otherwise =
    if isStuckInRoom as a
      then []
...
finishingMoves as = map makeMove as'
  where
    ...
    makeMove a =
      Move
        { _move = (_position a, getDestination as a),
          _amphipodType = _type a
        }
