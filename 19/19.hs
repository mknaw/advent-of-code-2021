...
instance Num Vector where
  ...
  (+) a b =  Vector { x=x b + x a
                    , y=y b + y a
                    , z=z b + z a }
  negate a = Vector { x= - x a
                    , y= - y a
                    , z= - z a }
...
(>=<) a b
    | a == flip b = True
    | otherwise   = a == b
  where flip v = Vector { x= - x v, y= - y v, z = - z v }
...
allRotations = [
  (         x,          y,          z), (         x, negate . z,          y),
  (         x, negate . y, negate . z), (         x,          z, negate . y),
  (negate . x, negate . y,          z), (negate . x, negate . z, negate . y),
  (negate . x,          y, negate . z), (negate . x,          z,          y),
  (         y, negate . x,          z), (         y, negate . z, negate . x),
  (         y,          x, negate . z), (         y,          z,          x),
  (negate . y, negate . x, negate . z), (negate . y, negate . z,          x),
  (negate . y,          x,          z), (negate . y,          z, negate . x),
  (         z, negate . x, negate . y), (         z, negate . y,          x),
  (         z,          x,          y), (         z,          y, negate . x),
  (negate . z, negate . x,          y), (negate . z, negate . y, negate . x),
  (negate . z,          x, negate . y), (negate . z,          y,          x)]
...
parseScanner input = initializeScanner id report
  where
    vectorify [x, y, z] = Vector { x=read x :: Int
                                 , y=read y :: Int
                                 , z=read z :: Int }
